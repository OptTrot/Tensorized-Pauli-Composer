...
%--------------------------------------------------------
In the paper, a new method to construct a Hamiltonian matrix 
which is given by weighted summation of Pauli operators.
The method is a combination of inversed TPD\cite{Hantzko_2024}
algorithm and symplectic tuple representation of generalized Pauli elements.
A proper canonical matrix to combined with inversed TPD algorithm is achieved 
by a simple binary XOR operation of common symplectic representation.
The algorithm has $O(n4^n)$ time complexity in worst case.
The symplectic representation allow us to express finite dimension Hilbert space 
as a matrix form, and with inversed TPD algorithm, common matrix representation is efficiently restored.
\end{abstract}
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        %\EndFor
        %\Comment{Construct canonical matrix}
